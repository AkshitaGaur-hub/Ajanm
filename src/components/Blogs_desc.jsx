...
import { fetchBlogById, fetchComments, addComment, resolveBlogImage } from '../services/api';
...
const COMMENTER_STORAGE_KEY = 'ajnam-commenter-info';
...
const Blogs_desc = () => {
    ...
    const [allIds, setAllIds] = useState([1, 2, 3, 4, 5]);
    const [customBlog, setCustomBlog] = useState(null);
    ...
    // If it is one of the original 5 blogs, use original matter object
    const initialBlog = matter.find((m) => m.id === blogId);
    const blog = initialBlog || customBlog;
    ...
    // Fetch comments and blog info from real database
    useEffect(() => {
        let isMounted = true;

        // Fetch comments for this blog from real DB
        ...
        // Fetch blog by id to get allIds and custom blog content if id > 5
            ...
            .then((data) => {
                ...
                if (!initialBlog && data) {
                    setCustomBlog({
                        id: data.id,
                        title1: data.title,
                        image: resolveBlogImage(data.image, data.id),
                        paragraphs: String(data.content || '').split('\n\n').filter(Boolean)
                    });
                }
            })
            .catch((err) => {
    ...
    const previousblog = () => {
        const index = allIds.indexOf(blogId);
        let prevId;
        if (index > 0) {
            prevId = allIds[index - 1];
        } else {
            prevId = allIds[allIds.length - 1] || 1;
        }
    ...
    const handleCommentSubmit = async (e) => {
        ...
        // Math captcha check: [input] × 3 = 18 => input must be 6
        if (Number(captchaInput.trim()) !== 6) {
            setFormError('Incorrect math captcha: what number × 3 = 18?');
            return;
        }
        ...
        try {
            ...
            setCommentText('');
            setCaptchaInput('');
            setFormSuccess('Your comment has been posted successfully!');
        } catch (err) {
            ...
            <div className="container">
                <div className="blog_content">
                    {initialBlog ? (
                        ...
                        <div className="img" key={blog.id}>
                            <img className="description_image w-full" src={blog.image} alt={blog.title1} />
                            <h2 className='head'>{blog.title1}</h2>
                            {blog.paragraphs?.map((p, idx) => (
                                <p className='para' key={idx}>{p}</p>
                            ))}
                        </div>
                    )}
                </div>
                    ...
                    <div className="comment-card">
                        ...
                        {comments.length > 0 && (
                            <div className="mb-8 pb-6 border-b border-gray-200">
                                <h3 className="comment-title text-2xl font-bold mb-4">Comments ({comments.length})</h3>
                        ...
                        <form onSubmit={handleCommentSubmit}>
                            ...
                            <div className="bottom-section">
                                ...
                                <button aria-label='post' type="submit" className="btn">
                                    Post Comment
                                </button>
                            </div>
                        </form>
