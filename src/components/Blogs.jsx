...
const Blogs = () => {
    ...
    // Synchronize with real database on mount
    React.useEffect(() => {
        let isMounted = true;
        fetchBlogs()
            .then((dbBlogs) => {
                if (!isMounted || !Array.isArray(dbBlogs)) return;

                // Sync the 5 initial blogs with real DB counts & likes
                const updatedInitial = initialBlogs.map((b) => {
                    const match = dbBlogs.find((item) => item.id === b.id);
                    return match ? { ...b, count: match.count, liked: match.liked } : b;
                });

                // Append any newly created blogs (id > 5) from the database
                const extraBlogs = dbBlogs
                    .filter((item) => item.id > 5)
                    .map((item) => ({
                        id: item.id,
                        title: item.title,
                        image: resolveBlogImage(item.image, item.id),
                        count: item.count || 0,
                        liked: item.liked || false,
                    }));

                setBlogs([...updatedInitial, ...extraBlogs]);
            })
            .catch((err) => {
                console.error('Error fetching blogs from real database:', err);
            });
    ...
    const toggleLike = async (event, id) => {
        ...
        try {
            ...
            console.error('Error updating like in real database:', err);
        }
    };
            ...
            <div className="blogs">
                <div className="blog_top">
                    ...
                    <div className="mt-4 flex justify-center">
                        <Link to="/add-blog">
                            <button className="add_blog_btn">Add Blog</button>
                        </Link>
                    </div>
                </div>
                ...
                <div className="blog_card">
                    {blogs.map((blog) => (
                        <Link to={`/blogs_desc/${blog.id}`} key={blog.id}>
                        <div className="card1">
                            <img
                                src={blog.image}
